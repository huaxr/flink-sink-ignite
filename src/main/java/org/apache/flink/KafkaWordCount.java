...
public class KafkaWordCount {
	...
	private final String kafkaConsumerGroup = "hxr_consumer";
	...
	public void consumerFromKafka() {
		...
		DataStream<String> wordcount = kafkaStream
			...
			}).name("flatmap") //this.transformation.setName(name);
			.keyBy(0)   // 使用keyBy(...)将会把你的无限stream拆分为逻辑上keyed的streams。如果没有调用keyBy(...)，你的stream就不是keyed。
